...
class ProvidersSearch:
    def __init__(
        ...
        *,
        ...
        fallback_resolvers = cast(list[SearchProvider], media_providers.values())

        self.main = provider_cls(metadata=metadata, clients=clients, providers=self)
        self.fallbacks = fallback_resolvers

    def fallback(self, query: str) -> Metadata:
        index = 0
        while index < len(self.fallbacks):
            provider = self.fallbacks[index]
            try:
    ...
    def filter_matching_domain_results(
        self, *, provider_results: Sequence[Metadata | Sentinel], youtube_results: None = None
    ) -> MatchingDomainResults: ...

    @overload
    def filter_matching_domain_results(
        self, *, provider_results: None = None, youtube_results: Sequence[YTVideoInfo | Sentinel]
    ) -> MatchingDomainResults: ...
        ...
        *,
        ...
        if youtube_results is not None:
            for index, video in enumerate(youtube_results):
                ...
                try:
                    spotify_search_result = self.main.search_track(
                        video_title
                    )
                except SongNotFound:
                    storage.new(
